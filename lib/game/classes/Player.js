export default class Player {
  ...
  collision(grid, player, context, game) {
    ...
    for (var i = 0; i < player.trail.length -20; i++) {
      if (player.trail[i][0] <= player.xPos &&
        player.trail[i][2] >= player.xPos &&
        player.trail[i][1] <= player.yPos &&
        player.trail[i][3] >= player.yPos) {
        console.log(player.trail[i][0], player.trail[i][2], player.xPos, "hit")
      }
    }
  }
